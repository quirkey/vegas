...
{% highlight ruby %}
Vegas::Runner.new(AppClass, 'app_name') do |opts, app|
  # opts is an option parser object
  # app is your app class
end
{% endhighlight %}

As of version 0.0.3, Vegas::Runner and its daemonization is fully compatible with Windows.

h4. Rack Apps

Vegas (> v0.1.0) is not dependent on Sinatra. Vegas works almost exactly the same for general Rack Applications. For example:
